...
for i=1:G_n_r
    ...
    sy=Coor(index,2);
    sz=Coor(index,3);
    [sN,sl,Az_gps,elv_gps,lat_IPP_gps,lon_IPP_gps,lat_sta,lon_sta,IPPZ_gps]=Get_GPSMatrix(fig,GPSP4,Gx,Gy,Gz,sx,sy,sz,G_n_r,C_n_r,G_n_s,C_n_s,i,K,M);
    N=N+sN'*sN*PG;
    U=U+sN'*sl*PG;
    %--RMS
...
for i=1:C_n_r
    ...
    sy=Coor(index,2);
    sz=Coor(index,3);
    [sN,sl,Az_bds,elv_bds,lat_IPP_bds,lon_IPP_bds,lat_sta,lon_sta,IPPZ_bds]=Get_BDSMatrix(fig,BDSP4,Cx,Cy,Cz,sx,sy,sz,G_n_r,C_n_r,G_n_s,C_n_s,i,K,M);
    N=N+sN'*sN*PC;
    U=U+sN'*sl*PC;
    %--RMS
    sizel=sizel+length(sl);
...
%% ------------------------------sub_function--------------------------------
function [MC,l,Az_dcb,elv_dcb,lat_IPP,lon_IPP,sb,sl,IPPZ]=Get_GPSMatrix(fig,GPSP4,x,y,z,sx,sy,sz,gps_n_r,bds_n_r,gps_n_s,bds_n_s,ith,K,M)
global Re h
MC=[];l=[];
...
%% ------------------------------sub_function--------------------------------
function [MC,l,Az_dcb,elv_dcb,lat_IPP,lon_IPP,sb,sl,IPPZ]=Get_BDSMatrix(fig,BDSP4,x,y,z,sx,sy,sz,gps_n_r,bds_n_r,gps_n_s,bds_n_s,ith,K,M)
global Re h
