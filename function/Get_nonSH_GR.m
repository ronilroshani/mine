...
for i=1:G_n_r
    ...
    sy=Coor(index,2);
    sz=Coor(index,3);
    [sN,sl,Az_gps,elv_gps,lat_IPP_gps,lon_IPP_gps,lat_sta,lon_sta,IPPZ_gps]=Get_GPSMatrix(fig,GPSP4,Gx,Gy,Gz,sx,sy,sz,G_n_r,R_n_r,G_n_s,R_n_s,i,K,M);
    N=N+sN'*sN*PG;
    U=U+sN'*sl*PG;
    %--RMS
...
for i=1:R_n_r
    ...
    sy=Coor(index,2);
    sz=Coor(index,3);
    [sN,sl,Az_glo,elv_glo,lat_IPP_glo,lon_IPP_glo,lat_sta,lon_sta,IPPZ_glo]=Get_GLOMatrix(fig,GLOP4,Rx,Ry,Rz,sx,sy,sz,G_n_r,R_n_r,G_n_s,R_n_s,i,K,M);
    N=N+sN'*sN*PR;
    U=U+sN'*sl*PR;
    %--RMS
    sizel=sizel+length(sl);
...
R_R=R(G_n_r+G_n_s+1:G_n_r+G_n_s+R_n_r)*10^9/c;
temp_glo=linspace(1,Rsat,Rsat);
temp_glo(glo_d_sat)=[];
R_S(temp_glo)=R(G_n_r+G_n_s+R_n_r+1:G_n_r+G_n_s+R_n_r+R_n_s)*10^9/c;
...
%% ------------------------------sub_function--------------------------------
function [MC,l,Az_dcb,elv_dcb,lat_IPP,lon_IPP,sb,sl,IPPZ]=Get_GPSMatrix(fig,GPSP4,x,y,z,sx,sy,sz,gps_n_r,glo_n_r,gps_n_s,glo_n_s,ith,K,M)
global Re h 
MC=[];l=[];
...
%% ------------------------------sub_function--------------------------------
function [MC,l,Az_dcb,elv_dcb,lat_IPP,lon_IPP,sb,sl,IPPZ]=Get_GLOMatrix(fig,GLOP4,x,y,z,sx,sy,sz,gps_n_r,glo_n_r,gps_n_s,glo_n_s,ith,K,M)
global Re h
