function[]=Modelling_VTEC_Bspline_manual(S_path,LAT_TKG,LON_TKG,tend)
...
for H =1:Hend
    ...
    for i=1:num_file
        ...
        if data_dir(i+2).name(1:3) == 'loc'
            ...
            [LAT_TKGz,LON_TKGz]=read_shp_iran_tehran_karaj;
            LAT_TK=[];LON_TK=[];
            kk=1;
            for kt=t1:t2
    ...
    worldmap([min(lat_lon_geo(:,1)) max(lat_lon_geo(:,1))],[min(lat_lon_geo(:,2)) max(lat_lon_geo(:,2))]);
    
    %     worldmap ([min(LAT_TKG)-2 max(LAT_TKG)+2],[min(LON_TKG)-2 max(LON_TKG)+2]);
    %     plotm(LAT_TKGz,LON_TKGz,'.','Color',[51 211 240]/256);hold on;
    ...
    %     figure_geoshow_manual_range(lon_map, lat_map, VTEC_map,['MAP_ TEC : [' num2str((t1*30/3600)) '-' num2str((t2*30/3600)) '] UT'],'[TECU]');
    tec=figure_geoshow_manual(lon_map, lat_map, VTEC_map,['MAP_ TEC : [' num2str((t1*30/3600)) '-' num2str((t2*30/3600))  ' UT]' '   date : ' num2str(dateVV(1,1)) '/' num2str(dateVV(1,2)) '/' num2str(dateVV(1,3))],'[TECU]',LAT_TKG,LON_TKG);
